		...
		for _, player in ipairs(game:GetService('Players'):GetPlayers()) do
			if player ~= localPlayer and (not IsTeamCheckEnabled or player.Team ~= localPlayerTeam) then
				...
				if humanoidRootPart then
					...
					humanoidRootPart.BrickColor = BrickColor.new("Black")
...
local infjump = false -- Assuming this is defined somewhere in your script
...
local tp = CFrame.new(0,0,0)

local load = function()

	game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = tp

	local Notification = GUI:Notification({ Title = "Teleport", Description = "Loaded Position", Time = "2" })
...
local save = function()
	...
	local Notification = GUI:Notification({ Title = "Teleport", Description = "Saved Position", Time = "2" })
...
local main = Library:Init { Title = "Z Battleground | Teamkel01" }
...
local button1 = GUI:Button({
	...
	Callback = function()
		task.wait(0.1)
        plr.Character.HumanoidRootPart.CFrame = CFrame.new(133, 176, -6)
	end,
}, section)
...
local button1 = GUI:Button({
	text = "Center",
	Callback = function()
		game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-547, 2658, 499)
	end,
}, section)
